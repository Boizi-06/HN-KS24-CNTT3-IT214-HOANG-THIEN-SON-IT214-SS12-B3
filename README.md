...
## KIỂM CHỨNG TÍNH ĐÚNG ĐẮN SAU KHI SỬA

1. **Trường hợp ban đêm (5 khách thanh toán đều lỗi):**
   - Đến request thứ 5: Tổng số call ghi nhận là 5.
   - Thỏa mãn điều kiện: $\text{Số calls } (5) \ge \text{minimumNumberOfCalls } (5)$.
   - Circuit Breaker tính toán: $\text{Failure Rate} = \frac{5}{5} \times 100\% = 100\% > 50\%$.
   - **Cầu dao LẬP TỨC CHUYỂN SANG TRẠNG THÁI OPEN trong 30 giây**. Mọi request tiếp theo bị ngắt kết nối ngay lập tức, bảo vệ hệ thống ngân hàng.
2. **Sau 30 giây:**
   - Circuit Breaker chuyển sang trạng thái **HALF_OPEN**.
   - Chỉ cho phép đúng **3 request** đi qua trinh sát (`permittedNumberOfCallsInHalfOpenState = 3`).
   - Nếu cả 3 request thành công, mạch chuyển lại về **CLOSED**. Nếu thất bại, mạch quay lại **OPEN** thêm 30 giây.
3. **Thỏa mãn nguyên tắc thiết kế:**
   - $\text{minimumNumberOfCalls } (5) \ge \text{permittedNumberOfCallsInHalfOpenState } (3)$: **Hợp lệ 100%**.
